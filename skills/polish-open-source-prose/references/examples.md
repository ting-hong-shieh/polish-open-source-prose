...
> CI 是發布前的必要關卡。若第三方 Action 或 Python 套件未固定版本，外部更新後，
> 同一個 commit 可能執行未經本專案審查的新版程式碼，使原本通過的 CI 日後失敗或
> 產生不同結果。為使 CI 的執行內容可追溯，並在失敗時重現相同環境，本測試會檢查
> 兩者是否使用固定版本。
>
> 測試只讀取本機檔案，不連網，也不解析 YAML，以免為了檢查 CI 而新增相依套件。

原因：原文的技術名詞本身沒有錯，但角色、風險、具體後果、測試目的與實作取捨擠在
兩句話中，因果關係不易辨識。修改後依序說明角色、風險、後果、防護方式與實作取捨。
不要機械地移除英文術語；只有當概念密度迫使讀者自行補出主詞、後果或因果關係時，
才拆句或補上連接。
